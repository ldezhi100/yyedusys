...
(function($) {
    $.fn.date = function(options, Ycallback, Ncallback) {
        ...
        $.fn.date.defaultOptions = {
            beginyear: nowdate.getFullYear()-100, //日期--年--份开始
        ...
        function resetInitDate() {
            ...
            initY = parseInt(that.val().substr(0, 4))-(nowdate.getFullYear()-100);
        ...
        function init_iScrll() {
            ...
            yearScroll = new iScroll("yearwrapper", {
                ...
                onScrollEnd: function() {
                    ...
                    var el = $("#yearwrapper ul li:eq(" + indexY + ")");
                     $("#yearwrapper ul .current").removeClass('current');
                     el.addClass('current');

                    strY = el.html().substr(0, $("#yearwrapper ul li:eq(" + indexY + ")").html().length - 1);
                    if (alreadyInit) {
            ...
            monthScroll = new iScroll("monthwrapper", {
                ...
                onScrollEnd: function() {
                    ...
                    if(parseInt(indexM)!==indexM){
                        return;
                    }
                    var el = $("#monthwrapper ul li:eq(" + indexM + ")");
                     $("#monthwrapper ul .current").removeClass('current');
                     el.addClass('current');
                    strM = el.html().substr(0, $("#monthwrapper ul li:eq(" + indexM + ")").html().length - 1);
                    ...
                    if (alreadyInit) {
                        ...
                        dayScroll.refresh();
                        (function(){
                            var indexD = (dayScroll.y / 40) * (-1) + 1;
                            var el = $("#daywrapper ul li:eq(" + indexD + ")");
                             $("#daywrapper ul .current").removeClass('current');
                             el.addClass('current');
                        }());
                    }
        ...
        function CreateDateUI() {
            var str = '' +
                ...
                '<div id="setcancle">' +
                '<div id="dateconfirm">确定</div>' +
                '<div id="datecancle">取消</div>' +
                '</div>' +            
                '<section>' +
